        ...
        <div className="relative min-h-[550px] sm:aspect-[4/5] md:aspect-[21/9]">
          ...
          <div className="absolute inset-0 flex flex-col justify-end p-6 md:p-10">
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
              <div>
                <span className="inline-block px-3 py-1 bg-neon-red text-white text-xs font-semibold uppercase tracking-wider rounded-full mb-4">
                  Prochain événement
                </span>
    ...
    <Link href={`/events/${event.id}`}>
      ...
      >
        <div className="event-card-image">
          ...
          {/* Price Tag */}
          <div className="absolute top-4 right-4 bg-neon-gold text-black px-3 py-1 text-sm font-bold rounded-full">
            {formatPrice(event.price)}
          </div>
          ...
          {(event.has_table_promo || event.has_table_vip) && (
            <div className="absolute bottom-4 left-4 flex gap-2">
              {event.has_table_promo && (
                <span className="bg-neon-red/80 text-white px-2 py-0.5 text-xs rounded">
                  Tables Promo
                </span>
              )}
              {event.has_table_vip && (
                <span className="bg-neon-gold/80 text-black px-2 py-0.5 text-xs font-medium rounded">
                  VIP
                </span>
              )}
            </div>
